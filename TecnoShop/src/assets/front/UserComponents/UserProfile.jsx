...
import { useApp } from "../../../context/AppContext"; // Cambiado a AppContext
import { Camera, Mail, Phone, MapPin, User, X } from "lucide-react";

const UserProfile = () => {
  const { user, isAuthenticated, loading } = useAuthUser();
  const { getUsuario, updateUsuario } = useApp(); // Funciones del AppContext
  ...
  useEffect(() => {
    const fetchUser = async () => {
      if (user?.id) {
        try {
          const userData = await getUsuario(user.id); // Usamos getUsuario desde AppContext
          if (userData) {
  ...
  const handleSave = async () => {
    try {
      const updatedUser = await updateUsuario(user.id, userInfo); // Usamos updateUsuario desde AppContext
      if (updatedUser) {
            ...
            <div className="relative -mt-24 mb-8 flex justify-center">
              <div className="relative group">
                <img
                  src={
                    profileImage ||
                    "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQv5ulumXxZdrcBKSu7AUfL2AfDlIs1fRSEFA&s"
                  }
                  alt="Perfil"
